...
import { DossierClientTabs } from "@/components/portail/dossier-notes-client";
...
export default async function DossierDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [dossierRes, timelineRes, leadRes, docsRes, msgsRes, notesRes] = await Promise.all([
    supabase
      .from("dossiers")
  ...
  const sharedNotes = (notesRes.data ?? []).map((n) => ({
    id: n.id as string,
