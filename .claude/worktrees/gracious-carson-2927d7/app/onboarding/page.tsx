...
// ---------------------------------------------------------------------------
// Wrapper with Suspense (requis pour useSearchParams)
// ---------------------------------------------------------------------------
...
function OnboardingWizard() {
  ...
  // Charger l'état depuis localStorage + récupérer l'org ID
  useEffect(() => {
    try {
      const saved = localStorage.getItem(STORAGE_KEY);
      if (saved) setWizard(JSON.parse(saved));
    } catch {}
  ...
  async function handleStep2(data: Step2Data) {
    ...
    startTransition(async () => {
      const fd = new FormData();
      fd.set("org_id",        orgId);
      fd.set("primary_color", data.primary_color);
      fd.set("accent_color",  data.accent_color);
      fd.set("logo_url",      data.logo_url);
  ...
  async function handleStep5() {
    ...
    startTransition(async () => {
      ...
      router.push(
        `/onboarding/success?__tenant=${tenant}&subdomain=${tenant}`
      );
    });
  }
...
function Step1({
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    onSubmit({ address, phone, website_url: website });
  }
  ...
  return (
    <StepShell
      ...
      subtitle="Ces informations apparaîtront sur votre portail client."
    >
      <form onSubmit={handleSubmit} className="space-y-5">
        <FormField
          id="address"
          label="Adresse"
          placeholder="Avenue Louise 480, 1050 Bruxelles"
          value={address}
        ...
        <FormField
          ...
          label="Site web"
          type="url"
          placeholder="https://votre-cabinet.be"
          value={website}
...
function Step2({
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    onSubmit({ primary_color: primary, accent_color: accent, logo_url: logoUrl });
  }
  ...
  return (
    <StepShell
      ...
      subtitle="Personnalisez votre portail aux couleurs de votre cabinet."
    >
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="space-y-3">
          ...
          <div
            className="flex items-center gap-4 rounded-sm border border-dashed p-6 cursor-pointer"
          ...
          >
            {logoUrl ? (
              ...
              <div
                className="flex h-12 w-12 items-center justify-center rounded-sm text-xs"
...
function Step3({
  ...
  initial?: Invite[];
  onSubmit: (d: Invite[]) => void;
  onBack: () => void;
  isPending: boolean;
}) {
  const [invites, setInvites] = useState<Invite[]>(
    initial ?? [{ email: "", role: "avocat" }]
  );

  function addRow() {
    setInvites((prev) => [...prev, { email: "", role: "avocat" }]);
  }
  ...
  return (
    <StepShell
      ...
      subtitle="Ajoutez vos collaborateurs. Ils recevront un email de connexion."
      optional
    >
...
function Step4({
  ...
  function updateRow(i: number, field: keyof Client, value: string) {
    setClients((prev) =>
      prev.map((row, idx) =>
        idx === i ? { ...row, [field]: value } : row
      )
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    const data = csvMode ? parseCsv(csvText) : clients.filter((c) => c.name && c.email);
    onSubmit(data);
  }
  ...
  return (
    <StepShell
      step={4}
      title="Importer vos premiers clients"
      subtitle="Créez les comptes de vos clients existants. Ils recevront un email d'accès."
      optional
    >
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="flex gap-3">
          <button
            ...
            style={{
              backgroundColor: !csvMode ? "var(--foreground)" : "transparent",
              borderColor: "var(--border)",
              color: !csvMode ? "#fff" : "var(--text-secondary)",
            }}
          >
            Formulaire
          </button>
        ...
        {csvMode ? (
          <div className="space-y-2">
            <p className="text-xs" style={{ color: "var(--text-muted)" }}>
              Format : <code>Nom Prénom, email@exemple.com</code> — un client par ligne
            </p>
            <textarea
              rows={8}
              placeholder={`Jean Dupont, jean.dupont@exemple.com\nMarie Martin, marie.martin@exemple.com`}
              value={csvText}
...
const PLANS = [
  {
    ...
    name:  "Solo",
    price: "49 €/mois",
    features: ["1 avocat", "10 dossiers actifs", "Portail client", "Messagerie sécurisée"],
  },
  {
    id:    "cabinet",
    name:  "Cabinet",
    price: "129 €/mois",
    features: ["Jusqu'à 5 avocats", "Dossiers illimités", "Signature électronique", "Formulaires intake"],
    highlight: true,
  },
  {
    id:    "premium",
    name:  "Premium",
    price: "299 €/mois",
    features: ["Avocats illimités", "IA résumés dossiers", "Domaine personnalisé", "Support prioritaire"],
  },
] as const;
...
function Step5({
  ...
  return (
    <StepShell
      ...
      subtitle="Vous êtes en période d'essai de 14 jours. Aucun paiement avant la fin de l'essai."
    >
      <div className="space-y-8">
        ...
        >
          ...
          <p className="text-sm" style={{ color: "var(--text-muted)" }}>
            ...
            <br />
            Votre période d&apos;essai de 14 jours est active dès maintenant.
          </p>
        </div>
    ...
    <div className="space-y-8">
      <div className="space-y-2">
        <p className="text-xs font-medium uppercase tracking-widest" style={{ color: "var(--text-muted)" }}>
          Étape {step} sur {TOTAL_STEPS}{optional ? " — optionnel" : ""}
        </p>
        <h1
      ...
      <div
        className="rounded-sm border px-8 py-8"
        style={{
          backgroundColor: "var(--surface)",
          borderColor: "var(--border)",
        }}
